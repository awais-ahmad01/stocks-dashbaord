...
const RealTimeDashboard = () => {
    ...
    return (
        <main className="max-w-7xl mx-auto min-h-screen px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-8">
                <h2 className="text-3xl font-bold text-gray-900 mb-2">Real-Time Stock Dashboard</h2>
                <p className="text-gray-600">Live market data updated every 1 minute.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                {stocksRealTimeData.map((stock) => (
                    <Link to={`/stockDetails/${stock.symbol}`} key={stock.symbol}>
                        ...
                        <button
                            onClick={(e) => handleAddToWatchlist(stock.symbol,stock.name, e)}
                            className="absolute top-4 right-4 p-2 text-gray-400 hover:text-blue-500 transition-colors duration-200"
